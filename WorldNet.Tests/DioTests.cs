...
public class DioTests
{
    ...
    public void EstimateMatchesReference()
    {
        ...
        double[] expectedPositions = ReferenceData.Load("dio_temporal_positions").Values;
        double[] expectedF0 = ReferenceData.Load("dio_f0").Values;
    ...
    public void GetSamplesForDioMatchesReference()
    {
        ...
        for (int r = 0; r < rates.Length; ++r)
        {
            for (int p = 0; p < periods.Length; ++p)
            {
                ...
                {
                    int actual = Dio.GetSamplesForDio(rates[r], lengths[l], periods[p]);
    ...
    public void EstimateRejectsNullArena()
    {
        ...
        Assert.Throws<ArgumentNullException>(
            () => Dio.Estimate(x, 44100, DioOption.Default, positions, f0, null!));
    }
    ...
    public void EstimateRejectsEmptyWaveform()
    {
        ...
        Assert.Throws<ArgumentException>(
            () => Dio.Estimate([], 44100, DioOption.Default, [], [], arena));
    }
    ...
    public void EstimateRejectsNonPositiveSampleRate()
    {
        ...
        Assert.Throws<ArgumentOutOfRangeException>(
            () => Dio.Estimate(x, 0, DioOption.Default, [], [], arena));
    }
    ...
    public void EstimateRejectsShortDestination()
    {
        using WorldArena arena = new();
        double[] x = new double[1000];
        int f0Length = Dio.GetSamplesForDio(44100, x.Length, DioOption.Default.FramePeriod);
        double[] positions = new double[f0Length];
        double[] f0 = new double[f0Length - 1];

        Assert.Throws<ArgumentException>(
            () => Dio.Estimate(x, 44100, DioOption.Default, positions, f0, arena));
    }
}
