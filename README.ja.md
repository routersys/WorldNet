...
MSVCで`/O2`を指定してビルドした原典C++と、本移植をNative AOTで発行したものとを比較します。どちらも事前コンパイルですので、条件を揃えた比較になります。どちらの表も両者を同一セッション内で連続して測っていますので、意味を持つ量は比であり、絶対値は計算機に応じて変動します。
...
二つ目の表はGitHub Actionsのランナー上で計測用のワークフローが再生成します。専用の計算機とは機種が異なりますので、比が特定の機械に依存しないことの独立した確認になります。
