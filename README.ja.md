...
- 処理負荷: Harvestが最も重く、次いでD4Cが重くなります。系列を得るだけであれば`Speed`を上げたDioが最も軽くなります。同一の環境でも実行ごとの変動が大きいため、本書に固定の数値は掲載しません。`WorldNet.Examples bench <input.wav>`が手元の環境での段別の所要時間を表示します。参照ハーネスも`WORLD_BENCH_ONLY`を設定すると、原典C++について同じ形式で表示します。
